"""一条命令跑完这个项目的全部对账，包括「正在跑的那个页面有没有说谎」。
    ...
    ./.venv/bin/python -X utf8 scripts/selfcheck.py                     # 默认十八条
...
另外十九条（`doctests`/`doctests-test`/`numbers`/`doc-cmds`/`names`/`self-test`/`doc-test`/`num-test`
/`unmarked`/`um-test`/`claims`/`claims-test`/`drift-test`/`epg-test`/`lean-test`/`heads`/`heads-test` 十七条默认，
...
**2.72 追记**：那两个数各再加二 —— 多的是 `heads` 与 `heads-test`（`scripts/doc_headings.py`
装进那一屏时屏幕上写的是「28 格」；第九把配对尺，守的是文档的标题结构那一层：编号连着排没有、
层级跳没跳、别处指过来的节号还在不在。今天它是 33 格，那一笔账在下面的 2.73 追记里）。这一遍是 2.60 那把尺**第三次**替我干活，而且只抓一处：两条步骤装完、我一句
说明书都没回头数，全树那一遍 `claims` 全场报「查到 25 处报数的话，**1 处对不上**」，那唯一
一处正是我刚写下的「`selfcheck.py:steps` 说 格 28、真值是 没点名，只要求属于
9/12/14/15/23/26/30/35」—— 与 2.64 那两把的来路一字不差：**新尺的真实格数不在别名表里，
就会被当成一个凭空的数**。补上 `doc_headings` 那一行别名之后同一分钟再跑全树，退 **0**。
那一行别名不是白加的，10:36:17 拿一个进程跑了三遍对照（`--py scripts/selfcheck.py`，
只量这一件，所以分母是 14 处不是上面那个 25）：装着别名退 **0**、只在内存里把那行摘掉退 **1**
且红回来**两处**（`模块` 与 `steps` 各一笔「28 格」—— 这一遍追记自己也写了一句 28，
它同样在被告状）、装回去退 **0** 且屏幕与第一遍**逐字相同**。
上一段那句代价这一遍仍然在：名单里那句「十六条默认」它还是读不到，那半句是我自己数的；
读得到的是 `模块`/`check_page`/`steps` 那几处「默认 17」「离线尺 16」—— 都是当场从
`steps()` 数出来的，不是我从屏幕上抄的。
**2.73 追记**：上面那句「28 格」从此只能带引号读 —— 那一把今天 33 格，本节装的就是它那一层：
`--docs` 一个人干了两份工（结构层查哪几篇、引用层拿哪几篇当靶子），拆成两份名单，并给
「靶子空了」那一档装一道**不许判**的闸。这一遍 2.60 那把尺**一处都没抓到**：那三处「28 格」
是我先改完才跑它的，而 `doc_headings` 那行别名 2.72 就装好了、允许的格数自己跟着基线走
（11:26:40 全树 25 处、0 处对不上、退 0，屏幕上那句已涨成「属于 9/12/14/15/23/26/30/33/35」）。
但「没抓到」不当记账成「没有过期数」，本节实录两处它管不着的：① `steps` 里那句按档分账
（十三／八／三／九）全用汉字写数，而 `GRID` 只认阿拉伯数字 —— 那四处从头到尾没进过任何一把尺
的眼，11:36:51 把 33 格逐格摊出来数过才对上（前一版写「三格」却没说是哪三格，那是查不动的三格）；
② `doc_headings.py` 说明书里那句「正文里有 1809 处」在本节开工**之前**就差 3 处（11:35:39 现数
**1812**），那 3 处正是 2.72 自己那 62 行正文落进文档时涨的 —— 而那一节的说明书当时就写着
「碍事的是下一节再抄一次上面那几个数」，本节就是那个下一节。两处同档：与上面那句「十六条默认」
一样，都是我数着改的，没人量。
"""
...
def check_page(port: int) -> tuple[str, str]:
    ...
    这一条量的那一层，是那十七条离线尺（`doctests`/`doctests-test`/`numbers`/`doc-cmds`/`names`/`self-test`
...
def steps(args: argparse.Namespace) -> list[tuple[str, str, Callable[[], tuple[str, str]]]]:
    ...
    默认十八条 —— 十七条离线尺（`doctests`/`doctests-test`/`numbers`/`doc-cmds`/`names`/`self-test`
    ...
    `heads` 与 `heads-test`（2.72）是第九把配对，也是这一屏上**第一次读文档的结构**：前八把
    各读各的一层（命令行、挂了标记的数、没挂标记的数、文件名、.py 的散文、两张表、一支生产件），
    「这一行是一条标题、它编的号连着没有」那一层一把都没碰。装它的理由不是「想要一把新尺」，
    是 2.71 收尾那句：那一节弄丢的 `## 三、总体架构` 在仓库里躺了一整节，而它记的原话是
    「一行标题的消失对那四把尺全是透明的」—— 最后是人的手在 `git diff --numstat` 里
    看见多出来的那个 −1。既然它已在仓库里躺过一次，
    「下次用眼睛盯」就不算对策。配对那一条与前八把同一笔账：今天它报「毛病 0 处」，
    而 0 有两种读法（文档真连着排 / 那几条判据整层不咬），所以那 33 格里十三格钉的是判据本身
    —— 十一格结构（断号／重号／串章／跳档／孤悬，今天的文档里一处实例都没有）、
    一格断链、一格「摊不完要说数」那句措辞；八格钉「读不到／参数取坏值」那一族，
    三格钉 2.73 那一族（`G8` 靶子与被查要分开、`B22`／`B23` 靶子空了就不许判），
    剩下九格钉的是「这些情形不许报错」（`G1`—`G7`、`G9`、`B16`）。
    各跑 0.09 秒与 0.07 秒（11:28:53 那两批各三遍：正查 0.09／0.09／0.09、
    基线 0.07／0.07／0.07 —— 2.73 之后 `heads` 把 `计划书.md` 读两遍（一遍当被查、一遍当靶子），
    这两个数没动；10:30:38 那批第一遍的 0.18 是缓存冷的）。
    `doctests-test`（2.74）是第十把配对尺，也是这一屏上**第一条量的不是「读数」而是「跑法」的**：
    前九把各自守一层内容（文件名、挂了标记的数、没挂标记的数、文档里的命令、.py 的散文、
    两张表、一支会写文件的生产件、文档的标题结构），这一把守的是 `run_doctests --each` 那一档
    **自己会不会说话** —— 它那一屏说出口的十几句（「字还在、路已死」「它去干了自己的活」
    ...
    本件此前那些用例喂给判据的全是**假输出**，那四层一次都没被量过（这一节第一遍跑基线时
...
'lean-test', 'heads', 'heads-test']
    ...
    out: list[tuple[str, str, Callable[[], tuple[str, str]]]] = [
        ...
        ("heads", "文档的标题编号连着排没有、别处指过来的节号还在不在",
...
def dispositions(fails: Sequence[str]) -> list[tuple[str, str]]:
    ...
    >>> # 2.72 那两条：`heads` 自己就有三种红，那句话不能替它猜是哪一种，只能把三种摆出来
    >>> [k for k, _ in dispositions(["heads"])]
    ['heads']
    >>> [k for k, _ in dispositions(["heads-test"])]
    ['heads-test', 'heads-test/heads-green']
    >>> [k for k, _ in dispositions(["heads", "heads-test"])]    # 两红：各一句，只少配对那句
    ['heads', 'heads-test']
    >>> # 2.74 那一条：收集器那一档的配对，`doctests` 在默认屏上，所以 `*-green` 那句配得上
    >>> [k for k, _ in dispositions(["doctests-test"])]
    ['doctests-test', 'doctests-test/doctests-green']
    >>> [k for k, _ in dispositions(["doctests", "doctests-test"])]   # 两红：只少配对那句
    ['doctests-test']
    ...
    ...      "heads", "heads-test", "doctests-test"]))
    True
...
'doc-test/doc-cmds-green']
    ...
    if "heads" in f:
        # 这一条最贵的误读是「文档里有标题坏了」—— 它只是三种红里的一种，而且多半不是它。
        out.append(("heads",
                    "`heads` 那条红有三种读法，屏幕上那三行各自点名，别在这儿猜："
                    "\n           写「结构毛病 N 处」是**编号自己断了**（断号／重号／串章／跳档／孤悬），改的是文档；"
                    "\n           写「引用断链 N 处」是**别处一句话指着一个文档里没有的节号**，改的是它点的那一行；"
                    "\n           写「N 篇没读到」则是分母只盖住读到的那几篇，改的是跑法（2.72）。"))
    if "heads-test" in f:
        out.append(("heads-test",
                    "`heads-test` 那条红**不是说文档里的标题坏了**：那 33 格是它自己种在临时目录里的，"
                    "\n           跑完就回收，仓库里那一篇它一个字没读。它说的是读标题那把尺的判据或措辞变了 —— "
                    "\n           先看它点的是 `G` 组（误伤）还是 `B` 组（该红没红），再 `git log -p scripts/doc_headings.py`（2.72）。"))
        if "heads" not in f:
            out.append(("heads-test/heads-green",
                        "`heads` 绿、`heads-test` 红 —— 这个组合只有一种意思：**那句「结构毛病 0 处、引用断链 0 处」"
                        "\n           这一轮不能读**。它可能是文档真连着排，也可能是那几条判据整层不咬 —— "
    ...
    if "doctests-test" in f:
        ...
        if "doctests" not in f:
            out.append(("doctests-test/doctests-green",
                        "`doctests` 绿、`doctests-test` 红 —— 这个组合只有一种意思：**手跑那一遍 `--each` 的读数"
                        "\n           这一轮不能读**。那一屏每一行是一句判决（「字还在、路已死」「它去干了自己的活」"
                        "\n           「那把尺判错了」……），而今天的仓库里那几句一句都没有实例 —— "
                        "\n           这一条是它们在这棵树里唯一的证据（2.71 那笔账，2.74 收到收集器自己头上）。"))
    return out
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--skip", action="append", default=[],
                    help="跳过某一步，可重复：page / numbers / doc-cmds / doctests / doctests-test / "
